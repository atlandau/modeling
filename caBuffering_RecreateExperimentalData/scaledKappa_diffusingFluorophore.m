...
fluorConcentrations = 1e-6*(50:10:300);
NC = length(fluorConcentrations);
fluorKappa = @(fProp) fProp.totalConcentration / (restConcentration + fProp.kd);

sols = cell(1,NC);
values = zeros(NT,2,NC); 
pkEstBuffer = zeros(NC,1);
for i = 1:NC    
    fluorProperties.totalConcentration = fluorConcentrations(i);
    cKappa = fluorKappa(fluorProperties);
    cInitialState = restConcentration*[1 cKappa];
    prmODE = @(t,y) instantScaledPlusFluorophores(t,y,spineVolume,extrusionRate,restConcentration,amplitude,cKappa,kappaKD,fluorProperties);
    sols{i} = ode23s(prmODE, tspan, cInitialState, odeOptions);
...
ylabel(cb,'[Fluor]');
colormap(rMap);
caxis(fluorConcentrations([1 end]));
set(gca,'ColorScale','log');
%legend(cellfun(@(c) sprintf('Kon: 1x10^{%d} /M/s',c),num2cell(log10(onRate)),'uni',0),'location','northeast');
set(gca,'fontsize',16);

subplot(1,3,2);
fluorResponse = squeeze(values(:,2,:));
dff = (fluorResponse - repmat(fluorResponse(1,:),NT,1)) ./ repmat(fluorResponse(1,:),NT,1);
dffNorm = dff ./ repmat(max(dff,[],1),NT,1);

toPlot = dff;
plot(1000*tvec, 1e6*toPlot(:,plotIdx), 'linewidth',1.5);
xlim([0 100]);
yLimBuffer = ylim;
xlabel('Time (ms)');
ylabel('然');
...
plot(fluorConcentrations, 1e6*pkCalcium, 'linewidth',1,'marker','o','color','k');
plot(fluorConcentrations, 1e6*pkEstBuffer, 'linewidth',1,'marker','o','color','r');
% set(gca,'xscale','log');
xlim(fluorConcentrations([1 end]));
yLIM = ylim;
% ylim([0 yLIM(2)]);
xlabel('Kappa Kd');
ylabel('[Ca^{2+}] (然)');
title('FluorConcentration vs. \Delta[Ca^{2+}]');
